...
function renderShelters(sheltersToRender) {
    ...
    shelterList.innerHTML = sheltersToRender.map(shelter => {
        ...
        return `
            <div class="shelter-card">
                ...
                <a href="shelter.html?shelterId=${shelter.id}" class="btn">View Pets</a>
            </div>
        `;
    }).join('');
}
...
function renderPets(petsToRender) {
    ...
    petList.innerHTML = petsToRender.map(pet => `
        <div class="pet-card">
            ...
            <button class="btn ${pet.status === 'Available' ? 'adopt' : 'return'} ${pet.status === 'Adopted' ? 'toggled' : ''}" 
